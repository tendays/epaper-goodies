...
public class WeatherWidget implements Widget, TemporalFace, CrownFace {
	...
	public WeatherWidget(Layout config, int hours) {
		this.config = config;
		this.hours = hours;
	}
	...
	private Range<ZonedDateTime> bound(Range<ZonedDateTime> base, Duration maxDuration) {
		ZonedDateTime maxUpperBound = base.lowerEndpoint().plus(maxDuration);
		if (base.upperEndpoint().isAfter(maxUpperBound)) {
			return Range.closed(base.lowerEndpoint(), maxUpperBound);
		} else {
			return base;
		}
	}
	...
	public void render(Drawable graphics, int w, int h) {
		...
		int plotLeft = 15;
		int fontHeight = 6;
		Range<ZonedDateTime> plotRange = bound(temperaturePlot.getTimeRange(), Duration.ofHours(hours));
		...
		/* Horizontal axis */
		ZonedDateTime minTime = plotRange.lowerEndpoint();
		ZonedDateTime maxTime = plotRange.upperEndpoint();
		...
		Coords pen = null;
		int lastIcon = Integer.MIN_VALUE;
		for (int i = 0; i < temperaturePlot.count(); i++) {
			DataPoint point = temperaturePlot.get(i);
			if (point.time.isAfter(plotRange.upperEndpoint())) {
				break; // don't render after end of plot
			}
			...
			if (icon != null) {
				Metrics.Size iconBounds = metrics.getIconBounds(icon).scale(2);
				int iconLeft = (int) to.getX() - iconBounds.width / 2;
				if (iconLeft > lastIcon) {
					int iconRight = iconLeft + iconBounds.width;
					int iconBottom = h;
					for (int j = 0; j < temperaturePlot.count(); j++) {
						var otherPoint = coordTransform.apply(temperaturePlot.get(j));
						if (otherPoint.getX() > iconRight) {
							break;
						} else if (otherPoint.getX() >= iconLeft) {
							iconBottom = Math.min(iconBottom, (int) otherPoint.getY());
						}
					}

					graphics.drawIcon(icon, new RectCoords(
							iconLeft + iconBounds.width / 2,
							iconBottom - iconBounds.height / 2), iconBounds.height, Drawable.Anchor.CENTRE, Drawable.ImageStyle.WHITE_TRANSPARENT);
					lastIcon = iconRight;
				}
			}
			pen = to;
